...
import { getFamilyMinistriesFromRole, isSuper, formatBirthday } from "@/lib/constants/roles";
import { FAMILY_MINISTRIES } from "@/lib/constants/ministries";

export default async function FamilyMinistryPage({ params }: { params: Promise<{ ministry: string }> }) {
  const { ministry } = await params;
  const session = await auth();
  if (!session) redirect("/");

  const { role } = session.user;
  const allowedMinistries = getFamilyMinistriesFromRole(role);

  // Access check: must be the head of this specific ministry or super
  if (!isSuper(role) && !allowedMinistries.includes(ministry)) redirect("/dashboard");

  const ministryData = FAMILY_MINISTRIES.find((m) => m.value === ministry);
  if (!ministryData) redirect("/dashboard");

  const res = await callAppsScript<SheetUser[]>("listUsers");
  const allUsers = res.data ?? [];
  const members = allUsers.filter((u) => u.family_ministry === ministry && u.account_type === "FEAST_ATTENDEE");
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5 pb-20 md:pb-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">{members.length} member{members.length !== 1 ? "s" : ""} in this ministry</p>
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="border-b border-gray-100 bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider whitespace-nowrap">Service Ministry</th>
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider whitespace-nowrap">Account</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {members.map((u) => (
                <tr key={u.email} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-xs text-gray-600 whitespace-nowrap">
                    {u.service_ministries
                      ? u.service_ministries.split(",").map((s) => s.trim().replace(/_/g, " ")).join(", ")
                      : "-"}
                  </td>
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-semibold ${
                      u.account_status === "ACTIVE_MEMBER" ? "bg-green-100 text-green-700" :
                      u.account_status === "FTD_PENDING_APPROVAL" ? "bg-yellow-100 text-yellow-700" :
                      "bg-gray-100 text-gray-500"
                    }`}>
              ...
              {members.length === 0 && (
                <tr><td colSpan={9} className="px-4 py-8 text-center text-sm text-gray-400">No members found in this ministry.</td></tr>
              )}
            </tbody>
          </table>
        </div>
